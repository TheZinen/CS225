 ...
 * 
 * Name: Jaylen Quizon
 * Description: Insert your meaningful description for the Password class in Lab 6.
 */
...
public class Password
{
	...
	// Method that returns the String password
	...
	public Password(String p1, String p2)
	{
		boolean foundUpper = false;
		boolean foundLower = false;
		boolean foundDigit = false;
		// Logic to check for possible exceptions/errors
		for (int i = 0; i < p1.length(); i++)
		{
			if (Character.isUpperCase(p1.charAt(i)))
				foundUpper = true;
			if (Character.isLowerCase(p1.charAt(i)))
				foundLower = true;
		...
		if (!(p1.length() >= 5))
			throw new IllegalPasswordException("ERROR: Password must be at least 5 characters long. Please try again.");
		
		if (!foundUpper)
			throw new IllegalPasswordException("ERROR: Password must have at least 1 uppercase letter. Please try again.");
		if (!foundLower)
			throw new IllegalPasswordException("ERROR: Password must have at least 1 lowercase letter. Please try again.");
		if (!foundDigit)
			throw new IllegalPasswordException("ERROR: Password must contain at least 1 number. Please try again.");
		
		if (!(p1.equals(p2)))
			throw new IllegalPasswordException("ERROR: Passwords must match. Please try again.");
