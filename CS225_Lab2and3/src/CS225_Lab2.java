...
public class CS225_Lab2
{
	...
	private void printRemainingPrizeBoard()
	{
		...
		// at the end (simple for loop)
			...
			for (double printPrizes : prizes)
				System.out.print(printPrizes + "   ");
			System.out.println("");
	...
	private double getBankerOffer()
	{
		...
		double overall = 0.0;
		double avgPrizes = 0.0;
		
		for (int i = 0; i < remainingPrizesBoard.size(); i++)
		{
			overall += remainingPrizesBoard.get(i);
		}
		avgPrizes = overall/remainingPrizesBoard.size();
		double bankerOffer = avgPrizes * 0.85;
	...
	private void chooseDoor(int door)
	{
		...
		else
		{
			...
			System.out.print("You have selected door number " + door + " which holds " + df.format(remainingPrizesBoard.get(door)));
			System.out.println("");
		...
		if (remainingPrizesBoard.size() == 1)
		{
			...
			gameIsOver = true;
			System.out.println("Congratulations, you've accepted the banker's offer of " + df.format(remainingPrizesBoard.get(0)));
			System.out.println("");
	...
	private void printOfferHistory(double acceptedPrize)
	{
		...
		System.out.println("!!!!!Congratulations, your final door contained " + remainingPrizesBoard + " !!!!!");
		
		double maxOffer = 0.0;
		System.out.println("***BANKER HISTORY***");
		for (int i = 0; i < offerHistory.size(); i++)
		{
			System.out.println("Offer " + (i + 1)+ ": " + df.format(offerHistory.get(i)));
			maxOffer = Math.max(maxOffer, offerHistory.get(i));
		}
		...
		if (maxOffer > acceptedPrize)
		{
			double lossMoneyAmnt = maxOffer - acceptedPrize;
			System.out.println("You left " + df.format(lossMoneyAmnt) + (" :"));
		}
		else 
		{
			System.out.println("Congratulations! You won more than the banker's max offer of " + df.format(maxOffer));
		}
	}
