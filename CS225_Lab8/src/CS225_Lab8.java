...
public class CS225_Lab8
{
	...
	private static Map<String, ArrayList<Integer>> getTableOfContents(String filename)
	{
		...
		//t Create the Map that you will eventually return and call it toc (for Table of Contents)
		...
		try
		{
			...
			while (scan.hasNext())
			{
				// Read in next word and update it so that we ignore periods, commas and parenthesis at the
				...
				{	
					...
					{
						// Get the current list of pages for this word and
					...
					{
						// Create a new list, add the current page to your list, and PUT
	...
	private static void printOrderedTableOfContents(Map<String, ArrayList<Integer>> toc)
	{
		...
		// Create a new ArrayList called orderedWords and add all
	...
	private static void printTopWordCounts(int numWords, Map<String, Integer> wordCount)
	{
		...
		{
		    public int compare(final WordCountPair wordCountPair1, final WordCountPair wordCountPair2) 
		    {
		    	return wordCountPair2.getCount() - wordCountPair1.getCount(); 
		    }
		});
